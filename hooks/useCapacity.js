...
const useCapacity = (data) => {
  ...
  const generate = async (capPlan, toWeek, fromWeek) => {
    let entries = await fetch(`api/capEntries/capPlan=${capPlan._id}`)
      .then((data) => data.json())
      .catch()

    let weeks = toWeek
      ? myWeeks.getWeekRange(capPlan.firstWeek, toWeek)
      : data.weeks
    ...
    if (fromWeek) {
      setOutput(
        newPlan.filter((weekly) => weekly.week.firstDate >= fromWeek.firstDate)
      )
    } else {
      setOutput(newPlan)
    }
  ...
  const getTable = (fields) => {
    ...
    let tableData = output.map((weekly) => [
      ...
      ...headers.map((header) =>
        weekly[header] || weekly[header] === 0
          ? Math.round(weekly[header] * 100) / 100
          : null
      ),
    ])
    ...
    return {
      data: {
        header: [
          "Week",
          "First Date",
          ...fields.map((field) => field.external),
        ],
        entries: tableData,
      },
      isConverted: true,
  ...
  const rawUpdate = async (capPlan) => {
    let language = data.languages.find(
      (language) => language._id === capPlan.language
    )
